...
class BIRNN_CRF(object):
    def __init__(self, num_classes, hidden_size=128, num_layers=2, batch_size=32,
        ...
        with tf.name_scope("birnn"):
            ...
            self.rnn_pred = tf.matmul(tf.reshape(self.outputs, [-1, 2 * hidden_size]), W) + b
            self.scores = tf.reshape(self.rnn_pred, [-1, tf.shape(self.outputs)[1], self.num_classes])
    ...
    def viterbi_decode(self, viterbi_decode, transition_params):
        """
        用于viterbi解码，用于tf之外
        :param viterbi_decode:
        :param transition_params:
        :return:
        """
        return tf.contrib.crf.viterbi_decode(viterbi_decode, transition_params)
